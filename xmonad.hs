...
-- GridSelect
myGSConfig = defaultGSConfig { gs_cellwidth = 780 }
...
keys' conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
    ...
    -- SPECIAL KEYS
    -- volume controls
    , ((modMask, xF86XK_AudioLowerVolume     ), runProcessWithInput "alsavol" ["-s","5","down"] [] >>= newalert)
    , ((modMask, xF86XK_AudioRaiseVolume     ), runProcessWithInput "alsavol" ["-s","5","up"] [] >>= newalert)
